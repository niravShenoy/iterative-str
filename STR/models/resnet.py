...
class ResNetWidthInc(nn.Module):
    def __init__(self, builder, block, layers, width=64, num_classes=1000):
        self.inplanes = width
        super(ResNetWidthInc, self).__init__()
        
        self.conv1 = builder.conv7x7(3, width, stride=2, first_layer=True)

        self.bn1 = builder.batchnorm(width)
        self.relu = builder.activation()
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(builder, block, width, layers[0])
        self.layer2 = self._make_layer(builder, block, 2 * width, layers[1], stride=2)
        self.layer3 = self._make_layer(builder, block, 4 * width, layers[2], stride=2)
        self.layer4 = self._make_layer(builder, block, 8 * width, layers[3], stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        # self.fc = nn.Linear(512 * block.expansion, num_classes)
        if args.last_layer_dense:
            self.fc = nn.Conv2d(8 * width * block.expansion, args.num_classes, 1)
        else:
            self.fc = builder.conv1x1(8 * width * block.expansion, num_classes)
...
# ResNet }}}
def ResNetWidth18Inc(pretrained=False, width=64, num_classes=1000):
    # TODO: pretrained
    return ResNetWidthInc(get_builder(), BasicBlock, [2, 2, 2, 2], width, num_classes)


def ResNetWidth50Inc(pretrained=False, width=64, num_classes=1000):
    # TODO: pretrained
    return ResNetWidthInc(get_builder(), Bottleneck, [3, 4, 6, 3], width, num_classes)